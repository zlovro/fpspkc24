...
namespace Source.Libraries.Kb2Lib.Mesh
{
    public static class Kb2MarchingCubes
    {
        private static readonly byte[][] mTriangulationTable =
        {
            new byte[]
            {
        ...
        private static readonly Vector3Int[] mVertexOffsets =
        {
            new(0, 0, 1), new(1, 0, 1), new(1, 0, 0), new(0, 0, 0), new(0, 1, 1), new(1, 1, 1), new(1, 1, 0),
            new(0, 1, 0)
        };

        // public static Mesh March(HvBitStream pMap, int pWidth, int pHeight, int pLength)
        // {
        //     return March((pPos => pMap.GetBool(pPos.y * Chunk.sizeX * Chunk.sizeZ + pPos.z * Chunk.sizeX + pPos.x)), pWidth, pHeight, pLength);
        // }

        private static readonly Vector3[] mEdgeOffsets =
        {
            new(0.5F, 0, 1), new(1, 0, 0.5F), new(0.5F, 0, 0), new(0, 0, 0.5F), new(0.5F, 1, 1), new(1, 1, 0.5F), new(0.5F, 1, 0),
            new(0, 1, 0.5F), new(0, 0.5F, 1), new(1, 0.5F, 1), new(1, 0.5F, 0), new(0, 0.5F, 0)
        };

        public static UnityEngine.Mesh March(Predicate<Vector3Int> pPredicate, int pWidth, int pHeight, int pLength)
        {
            ...
            for (int y = 0, vert = 0; y < pHeight - 1; y++)
            {
                for (var z = 0; z < pLength - 1; z++)
                {
                    for (var x = 0; x < pWidth - 1; x++)
                    {
                        var idx = 0;
                        for (var bit = 0; bit < 8; bit++)
                        {
                            idx |= (pPredicate.Invoke(new Vector3Int(x, y, z) + mVertexOffsets[bit]) ? 1 : 0) << bit;
                        }

                        foreach (var vertexIdx in mTriangulationTable[idx])
                        {
                            if (vertexIdx == 0xFF)
                            {
                                break;
                            ...
                            var vtx = new Vector3(x, y, z) + mEdgeOffsets[vertexIdx];
                            vertices.Add(vtx);
                            triangles.Add(vert++);
                        }
                    }
            ...
            Debug.Log($"Baked {vertices.Count} vertices");
